...
using Forge.Forms.AvaloniaUI.FormBuilding.Defaults.Initializers;
using Forge.Forms.AvaloniaUI.FormBuilding.Defaults.Properties;
using Forge.Forms.AvaloniaUI.FormBuilding.Defaults.Types;
using Forge.Forms.AvaloniaUI.FormBuilding.Xml;
using Position = Forge.Forms.AvaloniaUI.Annotations.Position;
...
public class FormBuilder : IFormBuilder
{
    ...
    /// </summary>
    public static readonly FormBuilder Default = new();
    ...
    public FormBuilder()
    {
        ...
        TypeConstructors =
            new Dictionary<string, Func<TypeConstructionContext, TypeConstructor>>(StringComparer.OrdinalIgnoreCase)
            {
    ...
    public IFormDefinition GetDefinition(Type type)
    {
        ...
        if (Primitives.TryGetValue(type, out var primitiveForm)) return primitiveForm;

        if (cachedDefinitions.TryGetValue(type, out var formDefinition)) return formDefinition;
    ...
    public FormDefinition GetDefinition(string xml)
    {
        return GetDefinition(xml, true);
    }

    public FormDefinition GetDefinition(string xml, bool freeze)
    {
        var document = XDocument.Parse(xml);
        if (document.Root == null) throw new InvalidOperationException("Invalid XML document.");
                        ...
                        if (from.StartsWith("type:"))
                        {
                            ...
                            itemsSource = propertyType ??
                                          throw new InvalidOperationException(
                                              $"Could not find type '{qualifiedType}'.");
    ...
    private FormDefinition BuildDefinition(Type type)
    {
        ...
        if (!type.IsClass || typeof(MulticastDelegate).IsAssignableFrom(type.BaseType)) return null;
        ...
        foreach (var attribute in type.GetCustomAttributes())
            switch (attribute)
            {
                ...
                case FormAttribute form:
                    mode = form.Mode;
                    grid = form.Grid;
                    if (grid == null || grid.Length < 1) grid = new[] { 1d };
                ...
                case MetaAttribute meta:
                    if (!string.IsNullOrEmpty(meta.Name)) formDefinition.Metadata[meta.Name] = meta.Value;
    ...
    {
        if (TypeDeserializers.TryGetValue(type, out var deserializer)) return deserializer;
    ...
    private static List<ElementRow> PerformLayout(double[] grid, List<ElementWrapper> elements)
    {
        ...
        foreach (var element in elements)
        {
            if (element.Column < 0)
                element.Column = 0;
            else if (element.Column >= gridLength) element.Column = gridLength - 1;

            if (element.ColumnSpan < 1)
                element.ColumnSpan = 1;
            else if (element.ColumnSpan > gridLength) element.ColumnSpan = gridLength;
                ...
                for (var i = 0; i < layout.Count; i++)
                {
                    ...
                    if (row.Sealed)
                    {
                        ...
                        for (j = i + 1; j < layout.Count; j++)
                        {
                            ...
                            if (nextRow.RowName != rowName) break;

                            if (nextRow.Sealed) continue;
                    ...
                    if (row.ColumnSpan >= gridLength) row.Sealed = true;
        ...
        foreach (var row in layout)
            if (row.RowName == null)
            {
                ...
                if (element.Column >= gridLength) element.Column = gridLength - 1;
            ...
            else
            {
                ...
                for (var i = 0; i < rowElements.Count; i++)
                {
                    ...
                    if (count > element.Column) element.Column = count;
                ...
                if (count > gridLength)
                {
                    ...
                    for (var i = gaps.Length - 1; i >= 0; i--)
                    {
                        ...
                        for (var j = i; j < gaps.Length; j++) rowElements[j].Column -= subtraction;
