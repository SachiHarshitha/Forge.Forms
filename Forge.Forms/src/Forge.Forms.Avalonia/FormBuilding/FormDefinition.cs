...
public class FormDefinition : IFormDefinition
{
    ...
    {
        if (ModelType != null) return Activator.CreateInstance(ModelType);

        if (!frozen) throw new InvalidOperationException("Cannot create dynamic models without freezing this object.");
