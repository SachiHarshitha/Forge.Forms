...
internal class ValidatorInitializer : IFieldInitializer
{
    ...
    {
        if (!(element is DataFormField dataField)) return;
        ...
        Func<string, object> deserializer)
    {
        ...
        switch (attribute.When)
        {
            ...
            case string expr:
                ...
                var notifyActivation = onActivation == ValidationAction.ClearErrors ||
                                       onActivation == ValidationAction.ValidateField;
                var notifyDeactivation = onDeactivation == ValidationAction.ClearErrors ||
        ...
        IValueConverter GetConverter(IResourceContext context)
        {
            ...
            if (converterName != null) converter = Resource.GetValueConverter(context, converterName);
    ...
    {
        if (argument is string methodName && !string.IsNullOrWhiteSpace(methodName)) return methodName;
    ...
    private static Func<ValidationContext, bool> GetMethod(Type type, string methodName)
    {
        ...
        bool IsMatch(MethodInfo methodInfo)
        {
            ...
            if (methodInfo.ReturnType != typeof(bool)) return false;
