...
public abstract class BindingProvider : ContentControl, IBindingProvider
{
    ...
    public BindingProxy this[string name]
    {
        get
        {
            if (proxyCache.TryGetValue(name, out var proxy)) return proxy;
    ...
    {
        if (FieldResources.TryGetValue(name, out var resource)) return resource.ProvideValue(Context);

        if (FormResources.TryGetValue(name, out resource)) return resource.ProvideValue(Context);

        if (ThrowOnNotFound) throw new InvalidOperationException($"Resource {name} not found.");
