...
public abstract class FieldValidator
{
    ...
    public ValidationResult Validate(object value, CultureInfo cultureInfo)
    {
        if (ValidationPipe != null)
        {
            ...
            if (ValidationPipe.Error != null) return ValidationResult.Success;

            // Optionally ignore null/empty values.
            if (IgnoreNullOrEmpty && (value == null || value is "")) return ValidationResult.Success;

            // Checking this later might be a tiny bit faster.
            if (!IsEnforced.Value) return ValidationResult.Success;
